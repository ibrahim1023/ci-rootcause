...
def test_action_entrypoint_rejects_invalid_mode(tmp_path: Path, monkeypatch) -> None:
    output_file = tmp_path / "github_output.txt"
    config_path = tmp_path / "ci-rootcause.yml"
    config_path.write_text(
        "\n".join(
    ...
    monkeypatch.setenv("INPUT_CONFIG_PATH", str(config_path))
    monkeypatch.setenv("INPUT_MAX_FIX_FILES", "5")
    monkeypatch.setenv("INPUT_MODE", "invalid-mode")
...
def test_action_entrypoint_rejects_agentic_full_without_explicit_opt_in(
    ...
    monkeypatch.setenv("INPUT_CONFIG_PATH", str(config_path))
    monkeypatch.setenv("INPUT_MAX_FIX_FILES", "5")
    monkeypatch.setenv("INPUT_MODE", "agentic_full")
    monkeypatch.setenv("INPUT_ENABLE_AGENTIC_FULL", "false")
    ...
    assert payload["pr_created"] == "false"
    assert payload["pr_failure_reason_code"] == "ACTION_INPUT_ERROR"
    assert "agentic_full requires explicit opt-in" in payload["pr_failure_reason"]


def test_action_entrypoint_safe_rollout_rejects_agentic_full_even_with_opt_in(
    tmp_path: Path, monkeypatch
) -> None:
    output_file = tmp_path / "github_output.txt"
    config_path = tmp_path / "ci-rootcause.yml"
    ...
    monkeypatch.setenv("INPUT_MAX_FIX_FILES", "5")
    monkeypatch.setenv("INPUT_MODE", "agentic_full")
    monkeypatch.setenv("INPUT_ENABLE_AGENTIC_FULL", "true")
    monkeypatch.setenv("INPUT_ROLLOUT_PROFILE", "safe-github-rollout")
    ...
    payload = _parse_github_output(output_file)
    assert payload["pr_failure_reason_code"] == "ACTION_INPUT_ERROR"
    assert "safe-github-rollout does not allow mode=agentic_full" in payload["pr_failure_reason"]
